...
typedef struct ConfigDefHelper {
  bool defined;
  char name[CONFIGURATION_LENGTH];
} ConfigDefHelper;
...
bool isEmpty(char *string) { return string == NULL || string[0] == '\0'; }
...
Machine Parse(Context *c, char *code) {
  ...
  for (int lineIndex = 0; lineIndex < lineCount; ++lineIndex) {
    ...
    if (isEmpty(symbol)) {
      error(c, "branch is missing match symbol (first parameter)", lineIndex);
    };
  ...
  for (int ci = 0; ci < MAX_CONF; ci++) {
      char name    =  configs[ci].name[0];
      bool defined = configs[ci].defined;
    if(name && !defined) {
        error(c, "a configuration is not properly defined", NOLINE);
    }
