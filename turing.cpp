...
Machine Parse(char *code) {
    ...
    for (int i = 0; i < lineCount; ++i) {
        ...
        assert(strcpy_s(lineToParse, sizeof(lineToParse), lines[i]) == 0);
        ...
        char *lineContext;
        char *name = strtok_s(lineToParse, configNameDelim, &lineContext);
        name = trim(name);
        char *body = strtok_s(NULL, configNameDelim, &lineContext);
        body = trim(body);
        ...
        for (int i = 0; i < branchCount; ++i) {
            ...
            char toParse[265];
            assert(strcpy_s(toParse, sizeof(toParse), branches[i]) == 0);

            char *branchContext;
            char *symbol = strtok_s(toParse, inBranchDelim, &branchContext);
            symbol = trim(symbol);
            char *opsString = strtok_s(NULL, inBranchDelim, &branchContext);
            opsString = trim(opsString);
            char *next = strtok_s(NULL, inBranchDelim, &branchContext);
            next = trim(next);
...
char *ReadSource(char *filename) {
    FILE *file;
    fopen_s(&file, filename, "rb"); // TODO Might be problematic outside of windows
